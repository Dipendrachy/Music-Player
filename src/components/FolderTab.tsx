...
export default function FolderTab({ songs, onPlaySong, onAddToPlaylist, hasActiveTrack = false }: FolderTabProps) {
  const [currentPath, setCurrentPath] = useState<string[]>([]); // empty represents root (Internal Storage + SD Card)
      ...
      <div className="px-5 py-4 border-b border-zinc-900 bg-zinc-950 dynamic-card dynamic-border">
        <div className="flex items-center justify-between mb-3.5">
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-sans font-semibold text-sm text-zinc-100">Folder Browser</h1>
          </div>
          {currentPath.length > 0 && (
            <div className="flex gap-1.5 text-xs">
              <button
                onClick={() => handlePlayFolder(activeNode, false)}
                className="px-2.5 py-1 bg-white hover:bg-zinc-100 text-black text-[11px] font-semibold rounded-lg flex items-center gap-1 transition-all shadow"
                title="Play folder sequentially"
              >
                <Play className="w-3 h-3 fill-black text-black" /> Play All
              </button>
              <button
                onClick={() => handlePlayFolder(activeNode, true)}
                className="px-2.5 py-1 bg-zinc-900 hover:bg-zinc-850 text-zinc-200 border border-zinc-800 text-[11px] font-semibold rounded-lg flex items-center gap-1 transition-all"
        ...
        {/* Path breadcrumbs */}
        <div className="flex items-center gap-1.5 overflow-x-auto text-[10px] font-mono py-1 scrollbar-none text-zinc-500">
          <button 
      ...
      <div className={`flex-1 overflow-y-auto px-4 py-2 space-y-1.5 scrollbar-none ${hasActiveTrack ? 'pb-40' : 'pb-24'}`}>
        ...
        {currentPath.length > 0 && (
          <button
            ...
            className="w-full flex items-center gap-3 p-3 hover:bg-zinc-900/40 rounded-xl transition-colors text-left"
          >
            <Folder className="w-5 h-5 text-zinc-650 fill-zinc-950" />
            <div>
              <h3 className="font-semibold text-xs text-zinc-500">..</h3>
              <p className="text-[9px] text-zinc-650">Go to parent folder</p>
            </div>
          </button>
        )}
        ...
        {foldersAtLevel.map((fold) => {
          ...
          return (
            <div
              key={fold.fullPath}
              className="flex items-center justify-between p-3 bg-zinc-950/40 hover:bg-zinc-900/50 rounded-xl transition-all border border-zinc-900/40 hover:border-zinc-800 group"
            >
              <button
                onClick={() => setCurrentPath(fold.fullPath.split('/'))}
                className="flex-1 flex items-center gap-3.5 text-left"
              >
                <div className="p-2 bg-zinc-900 text-zinc-400 rounded-lg group-hover:bg-zinc-850 transition-colors">
                  <Folder className="w-4.5 h-4.5 fill-current" />
                </div>
                <div>
                  <h3 className="font-semibold text-xs text-zinc-200 group-hover:text-white transition-colors">{fold.name}</h3>
                  <p className="text-[9px] text-zinc-500 font-mono">{count} {count === 1 ? 'song' : 'songs'}</p>
                </div>
              </button>
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="p-1.5 rounded-full text-zinc-500 hover:text-white hover:bg-zinc-900 transition-all"
                  title="Shuffle folder"
                >
                ...
                <div className="relative">
                  ...
                  {showPlaylistSelector === fold.fullPath && (
                    <div className={`absolute right-0 mt-2 w-48 rounded-xl p-1.5 z-40 shadow-2xl animate-fade-in text-[11px] border ${
                      isLight 
                      ...
                      <span className={`block px-2.5 py-1 font-bold uppercase text-[9px] border-b mb-1.5 ${
                        isLight ? 'text-zinc-500 border-zinc-100' : 'text-zinc-500 border-zinc-900/60'
                      }`}>
                        Add to playlist
                      </span>
                      {playlists.length === 0 ? (
                        <span className={`block p-2 text-center ${isLight ? 'text-zinc-400' : 'text-zinc-600'}`}>No custom playlists created</span>
                      ) : (
                        playlists.map((pl) => (
                          <button
                            key={pl.id}
        ...
        {filesAtLevel.map((file) => (
          ...
          >
            ...
            >
              <SongCover song={file} className="w-8 h-8 rounded-sm shrink-0" size="sm" />
              <div className="truncate">
                <h4 className="font-semibold text-xs text-zinc-200 group-hover:text-white transition-colors truncate">{file.title}</h4>
